...
USE master
GO
IF EXISTS(SELECT NAME FROM sysdatabases WHERE NAME = 'PVBClothing')
BEGIN
	DROP DATABASE PVBClothing
END
GO
CREATE DATABASE PVBClothing 
GO
USE PVBClothing
GO
-- Tạo bảng
CREATE TABLE [dbo].[Product]( --Keep--
	...
	[userName] VARCHAR(50) NOT NULL --adjust memberId
)
GO
CREATE TABLE [dbo].[ProductCategory]( --keep
	[categoryId] INT IDENTITY PRIMARY KEY([categoryId]),
	[categoryName] NVARCHAR(250) NOT NULL,
	[note] nvarchar(128),
)
GO
CREATE TABLE [dbo].[Member]( --adjust
	[userName]	VARCHAR(50) PRIMARY KEY([userName]),--adjust
...
GO
CREATE TABLE [dbo].[Role](--keep
	[roleId] INT IDENTITY PRIMARY KEY([roleId]),
	[roleName] NVARCHAR(50) NOT NULL,
	[note] nvarchar(130)
)
GO
CREATE TABLE [dbo].[Customer](--remove
...
GO
CREATE TABLE [dbo].[Article]( --remove
...
INSERT INTO [dbo].[Member] ([userName], [password], [firstName], [lastName], [email], [birthday], [gender], [phone], [address], [avatar], [status], [roleId]) VALUES (N'admin', N'admin', N'Võ', N'Bảo', N'vbao964@gmail.com', CAST(N'2001-04-19' AS Date), 1, N'012365238', N'Dương Bá Trạc, TPHCM', N'~/Content/img/avatar/avatar.jpg', 1, 1)
select * from Member
...
select * from dbo.Product where categoryId = 3
